...
qfa.Hierachical<-function(experiment,Scaling,iter,upd,thin,inits,PlotOutput=TRUE,work,CustomModel=FALSE,NoOfRep){
a<-experiment
a<-funcIDORDER(a)
IDuni<-unique(a$ID)
ORFuni<-unique(a$ORF)
...
samp<-funcFITandUPDATE(QFA.I,QFA.D,QFA.P,inits,NoOfRep)
